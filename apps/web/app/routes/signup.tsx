...
export async function action({ request }: Route.ActionArgs) {
  ...
  // Default cycle calendar: 5-day cycle, school year starts first Monday of Sep, ends ~10 months later.
  const now = new Date();
  ...
  const schoolYearEnd = new Date(schoolYearStart.getTime() + 305 * 86_400_000); // +10 months
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const [user] = await tx
        ...
        .values({
          ...
          name: adminName,
          role: 'school_admin',
          emailVerifiedAt: new Date(), // skip verification flow for trial
        })
        .returning();
